...
class LLMJudge:
    ...
    def _init_client(self, config: dict):
        ...
        try:
            ...
            endpoint = eval_config.get('endpoint', '')
            api_key = eval_config.get('api_key', '')
            
            if not endpoint or not api_key:
                logger.warning(f"LLM Judge: missing endpoint or api_key in evaluation.azure_openai")
                logger.warning(f"  endpoint: {'set' if endpoint else 'MISSING'}")
                logger.warning(f"  api_key: {'set' if api_key else 'MISSING'}")
                return
            
            # Check if keyvault reference was not resolved
            if '@azureKeyVault' in str(endpoint) or '@azureKeyVault' in str(api_key):
                logger.warning("LLM Judge: azure_openai credentials still contain @azureKeyVault() - not resolved")
                return
