...
if 'pdf_content' not in st.session_state:
    st.session_state.pdf_content = None # To store the pdf content
...
if confidence_selection != st.session_state.confidence_selection:
    ...
    st.session_state.pdf_content = None # Clear pdf
...
def show_pdf(source_blob):
    """Display the PDF document."""
    if st.session_state.pdf_content is None or st.session_state.pdf_source_blob != source_blob:
        pdf_content = download_blob_to_memory(blob_service_client, container_name, source_blob)
        st.session_state.pdf_content = pdf_content
        st.session_state.pdf_source_blob = source_blob # store source blob to session
        ...
        if csv_df is not None:
            ...
            if st.button("Save Edits"):
                ...
                pdf_success, pdf_url = upload_to_blob_storage(
                    ...
                    st.session_state.pdf_content, # use stored pdf content
                    "application/pdf"
                )
